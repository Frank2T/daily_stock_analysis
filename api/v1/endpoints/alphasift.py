...
def _call_alphasift_screen(screen: Any, strategy: str, market: str, max_results: int, config: Config) -> Any:
    ...
    with _alphasift_runtime_env(config), _alphasift_dsa_daily_history_provider():
        try:
...
def _alphasift_dsa_daily_history_provider() -> Iterator[None]:
    ...
    def fetch_daily_history_with_dsa(
        ...
        *,
        ...
        try:
            dsa_df, dsa_source = _load_dsa_daily_history(code, lookback_days=lookback_days)
...
def _load_dsa_daily_history(code: str, *, lookback_days: int) -> Tuple[Any, str]:
    from src.services.history_loader import load_history_df

    normalized_code = _env_text(code).zfill(6)
